		...
		<div flex="dir:left cross:center" class="YY-information">
 		...
 		<ul class="YY-business-list">
			...
			<li flex="dir:left cross:center">
				...
				<select class="YY-fselect" v-model='choose_sale' v-if="categorys!=null">
				 	<option v-for="item in categorys" :value="item.sale_form_id" >{{item.sale_name}}</option>
				</select>
			</li>
		    <template v-for="(items,index) in commodity" v-if="items.parameter_group_id == choose_sale ">
		        <li  flex="dir:left cross:center" v-for="item in items.parameter_list">
					...
					<input style="width:130px;" type="text" name="" v-model='item.commodity_parameter_value' v-else :disabled="item.commodity_parameter_name=='商品重量'||item.commodity_parameter_name=='当日金价'||(item.commodity_parameter_name=='最终价格'&&item.parameter_group_id==3)">
					<p style="margin-left:10px;">
						<span>{{item.commodity_parameter_unit}}</span>
						<span v-if="item.commodity_parameter_name=='最终价格'&&item.parameter_group_id==3">(当日金价x商品重量+加工费)</span>
					</p>
		        </li>
		    </template>
	...
	export default {
		...
		computed:{
			sale_from_list:function(){
				let put=[]
				put.push(this.choose_sale)
				return put
			}
		},
	    created(){
	    	CommonApi.category(this,1,null,2)
          	commonlistApi.GetSaleFormList(this,2)
	    ...
	    watch:{
	    	...
	    	choose_brand:function(val,oldVal){
	    		...
	    		SeriesApi.getList_All(this,val,2)
	    	},
    		choose_series:function(val,oldVal){
    			...
    			StyleApi.getList_All(this,val,2)	
    		},
		    ...
		    'commodity':{
		    	handler(curVal,oldVal){
		    		...
		    		let weight=null
		    		let add=null
		    		let final_money=null
		    		for(let item of curVal){
		    			if(item.parameter_group_id==3){
			    			for(let items of item.parameter_list){
			    				if(items.commodity_parameter_name=="加工费"){
			    					if(items.commodity_parameter_value==''||items.commodity_parameter_value==null){
			    						add=0
			    					}else{
			    						add=parseInt(items.commodity_parameter_value)		    						
			    					}
			    				}
			    				if(items.commodity_parameter_name=="当日金价"){
			    					day_money=parseInt(items.commodity_parameter_value)
			    				}
			    				if(items.commodity_parameter_name=="商品重量"){
			    					if(items.commodity_parameter_value==''||items.commodity_parameter_value==null){
			    						weight=0
			    					}else{
			    						weight=parseInt(items.commodity_parameter_value)		    						
			    					}
			    				}
			    			}			
		    			}
		    		}
　　　　　　　　　　final_money=day_money*weight+add
					for(let item of curVal){
		    			if(item.parameter_group_id==3){
			    			for(let items of item.parameter_list){
			    				if(items.commodity_parameter_name=="最终价格"){
			    					items.commodity_parameter_value=final_money
			    				}
			    			}			
		    			}
		    		}
